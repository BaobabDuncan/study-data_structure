...
void printSimCustomer(int currentTime, SimCustomer customer) {
    ...
    switch (customer.status) {
        ...
        case start:
            printf("start");
            printf(",arrivalTime-%d,(startTime-arrivalTime)-%d\n", customer.arrivalTime,
                   (customer.startTime - customer.arrivalTime));
            break;
        case end:
            printf("end");
            printf(",arrivalTime-%d,startTime-%d,(startTime-arrivalTime)-%d,(endTime-startTime)-%d\n",
...
void printWaitQueueStatus(int currentTime, LinkedQueue *pWaitQueue){
    printf("[%d],currentElementCount: %d\n",
           currentTime,
...
void printReport(LinkedQueue *pWaitQueue,
    ...
    if (serviceUserCount > 0) {
        printf("totalWaitTime / serviceUserCount: %f\n",
